...
def get_reward_v2(action_time, vehicle_intersection_times):
    wait_times: List[float] = []
    for r in vehicle_intersection_times:
        car_entrance_time, car_exit_time = r[2], r[3] if r[3] else float("inf")
        if action_time <= car_exit_time <= action_time + REWARD_INTERVAL:
...
def get_env_state(action_time, snapshot_data, snapshot_time_to_index, num_snapshots):
    ...
    if i < 4:
        ...
        return torch.hstack([x[1] for x in snapshot_data[i-num_snapshots+1:i+1]])
...
def post_process(dataFolder):
    ...
    snapshot_data = [(-1.0, torch.zeros(10)) for _ in range(20)] + snapshot_data
    snapshot_time_to_index = {t[0]:i for i,t in enumerate(snapshot_data)}
    all_sars = []
    state = get_env_state(actions[0][0], snapshot_data, snapshot_time_to_index, num_snapshots=20)
    for a in actions[1:]:
        next_state = get_env_state(a[0], snapshot_data, snapshot_time_to_index, num_snapshots=20)
        action = a[1]
        reward = REWARD_FUNC(a[0], vehicle_intersection_times)
        all_sars.append((state, action, reward, next_state))
        state = next_state
